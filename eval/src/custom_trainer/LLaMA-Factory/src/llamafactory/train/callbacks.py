    ...
    def on_train_begin(
        ...
        **kwargs,
        ...
        if "wandb" in args.report_to:
            ...
            wandb.init(project=os.getenv("WANDB_PROJECT"), name=args.run_name)
...
class PPOSaveIntermediateCallback(TrainerCallback):
    r"""
    A callback for saving the intermediate PPO Stats
    """

    def __init__(
        self,
        ppo_log_interval: int,
        ppo_save_interval: int,
        save_name: str = "PPO_stats.jsonl",
    ) -> None:
        self.stats = []
        self.save_name = save_name
        self.ppo_log_interval = ppo_log_interval
        self.ppo_save_interval = ppo_save_interval
        self.save_counter = 0

    def save_jsonl(self, args: "TrainingArguments"):
        save_name = f"{self.save_counter}_{self.save_name}"
    ...
    def log_jsonl(self, state: "TrainerState"):
        stats_dict = state.current_ppo_stat
        stats_dict["step"] = state.global_step
    ...
    def on_step_end(
        ...
        **kwargs,
        ...
        if state.global_step % self.ppo_log_interval == 0:
            self.log_jsonl(state)

        if state.global_step % self.ppo_save_interval == 0:
            self.save_jsonl(args)
