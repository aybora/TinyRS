...
def create_reward_model(
    ...
    if finetuning_args.reward_model_type == "api":
        ...
        if finetuning_args.custom_reward_model:
            try:
                tokenizer = load_tokenizer(reward_model_args)["tokenizer"]
            except Exception as e:
                logger.warning_rank0(
                    f"Failed to load tokenizer from {reward_model_args.model_name_or_path}, using the tokenizer from the model itself."
                )
                tokenizer = load_tokenizer(model_args)["tokenizer"]

            reward_model = load_custom_model(
                tokenizer,
                reward_model_args,
