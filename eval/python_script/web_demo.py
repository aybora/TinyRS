...
def _parse_reasoning_text(text):
    try:
        # Extract think part
        ...
        def predict(_chatbot, task_history):
            ...
            for q, a in history_cp:
                if isinstance(q, (tuple, list)):
                    ...
                    if args.reasoning and len(messages) == 0:
                        messages.append(
                            {
                                "role": "system",
                                "content": [
                                    {"type": "text", "text": REASONING_SYSTEM_MESSAGE}
                                ],
                            }
                        )
            ...
            for response in call_local_model(model, processor, messages):
                _chatbot[-1] = (
                    ...
                    (
                        ...
                        else _remove_image_special(_parse_reasoning_text(response))
                    ),
                )

                yield _chatbot
                full_response = (
                    _parse_text(response) if not args.reasoning else response
                )
...
<center><font size=3>This WebUI is based on Qwen2-VL, developed by Alibaba Cloud.</center>"""
        ...
        chatbot = gr.Chatbot(
            label="Qwen2-VL", elem_classes="control-height", height=500
        )
