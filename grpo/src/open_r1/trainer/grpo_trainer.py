...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        seed = (base
                ...
                + self._micro_step)                    # 0 or 1 when grad-acc=2
        ...
        global_rewards = self.accelerator.gather(rewards)          # (world_B * G,)
        # ===============================================================

        # -------------------------------------------------------------------
        # Group-wise statistics – now computed over the global batch
        # -------------------------------------------------------------------
        global_rewards = global_rewards.view(-1, self.num_generations)   # (128, 4)
        mean_grouped_rewards = global_rewards.mean(dim=1)                # (128,)
        std_grouped_rewards  = global_rewards.std(dim=1)

        # slice back the part that belongs to *this* rank so shapes match
        rank      = self.accelerator.process_index
        world_bs  = global_rewards.size(0)                # 128
        local_bs  = world_bs // self.accelerator.num_processes
        start, end = rank * local_bs, (rank + 1) * local_bs
        mean_grouped_rewards = mean_grouped_rewards[start:end].repeat_interleave(self.num_generations)
        std_grouped_rewards  = std_grouped_rewards[start:end].repeat_interleave(self.num_generations)
        ...
        self._micro_step += 1
        if self._micro_step == self.args.gradient_accumulation_steps:
            self._micro_step = 0
